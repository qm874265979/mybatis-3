...
public class ReuseExecutor extends BaseExecutor {
  ...
  public <E> List<E> doQuery(MappedStatement ms, Object parameter, RowBounds rowBounds, ResultHandler resultHandler, BoundSql boundSql) throws SQLException {
    ...
    // <1> 调用 #prepareStatement(StatementHandler handler, Log statementLog) 方法，初始化 StatementHandler 对象
    Statement stmt = prepareStatement(handler, ms.getStatementLog());
  ...
  private Statement prepareStatement(StatementHandler handler, Log statementLog) throws SQLException {
    ...
    // 调用 #hasStatementFor(String sql) 方法，判断是否存在对应的 Statement 对象
    if (hasStatementFor(sql)) {
      // <1.1> 调用 #getStatement(String s) 方法，从缓存中获得 Statement 或 PrepareStatement 对象
      stmt = getStatement(sql);
      // <1.2> 调用 #applyTransactionTimeout(Statement stmt) 方法，设置事务超时时间
    ...
    // <2> 调用 StatementHandler#prepare(Statement statement) 方法，设置 SQL 上的参数，例如 PrepareStatement 对象上的占位符
  ...
  private boolean hasStatementFor(String sql) {
    try {
      ...
      return statement != null && !statement.getConnection().isClosed();//要求连接未关闭。

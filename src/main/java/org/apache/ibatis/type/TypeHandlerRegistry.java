...
public final class TypeHandlerRegistry {
  ...
  public TypeHandlerRegistry(Configuration configuration) {
     ...
     *   JDBC_TYPE_HANDLER_MAP 是一一映射，简单就可以获得 JDBC Type 对应的 TypeHandler ，而 TYPE_HANDLER_MAP 是一对多映射，
     *  一个 JavaType 怎么获取到对应的 TypeHandler 呢？答案在 {@link #getTypeHandler(Type type, JdbcType jdbcType)}方法。
     */
  ...
  public <T> void register(TypeHandler<T> typeHandler) {
    ...
    // <7> 最差，使用 Java Type 为 null 进行注册。但是，这种情况下，只会将 typeHandler 添加到 ALL_TYPE_HANDLERS_MAP 中。因为，实际上没有 Java Type 。
    if (!mappedTypeFound) {
  ...
  private <T> void register(Type javaType, TypeHandler<? extends T> typeHandler) {
    ...
    if (mappedJdbcTypes != null) {
      ...
      if (mappedJdbcTypes.includeNullJdbcType()) {
        // <5>发起注册时，jdbcType 参数为 null ，这是为啥？

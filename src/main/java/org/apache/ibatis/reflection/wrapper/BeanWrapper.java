...
public class BeanWrapper extends BaseWrapper {
  ...
  public Class<?> getGetterType(String name) {
    ...
    // 有子表达式
    if (prop.hasNext()) {
      // <1> 创建 MetaObject 对象。基于当前属性，创建 MetaObject 对象。如果该属性对应的值为空，那么 metaValue 会等于 SystemMetaObject.NULL_META_OBJECT 。也因为为空，那么就不能基于 metaValue 去做递归，获取返回值的类型。
      MetaObject metaValue = metaObject.metaObjectForProperty(prop.getIndexedName());

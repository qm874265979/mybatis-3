...
public class Configuration {
  ...
  /**
   * 解析失败待定的XMLStatementBuilder 集合
   */
  protected final Collection<XMLStatementBuilder> incompleteStatements = new LinkedList<>();
  /**
   * 解析失败待定的CacheRefResolver 集合
   */
  protected final Collection<CacheRefResolver> incompleteCacheRefs = new LinkedList<>();
  /**
   * 解析失败待定的ResultMapResolver 集合
   */
  ...
  public Executor newExecutor(Transaction transaction, ExecutorType executorType) {
    // <1> 获得执行器类型。可以通过在 mybatis-config.xml 配置文件：<setting name="defaultExecutorType" value="" /> value 有三种类型：SIMPLE REUSE BATCH
  ...
  // Slow but a one time cost. A better solution is welcome.遍历全局的 ResultMap 集合，若其拥有 Discriminator 对象，则判断是否强制标记为有内嵌的 ResultMap
